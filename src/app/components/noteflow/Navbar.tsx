    ...
    >
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#0F1117]/95 backdrop-blur-xl border-b border-white/5 px-4 py-4 flex flex-col gap-3">
          <button onClick={() => scrollTo("features")} className="text-left text-sm text-white/70 hover:text-white py-2">{tx.features}</button>
          <button onClick={() => scrollTo("how-it-works")} className="text-left text-sm text-white/70 hover:text-white py-2">{tx.howItWorks}</button>
          <button onClick={() => scrollTo("pricing")} className="text-left text-sm text-white/70 hover:text-white py-2">{tx.pricing}</button>
          <a href="/dashboard" className="text-sm text-white/70 hover:text-white py-2">{tx.dashboard}</a>
          <div className="flex items-center gap-3 pt-2">
            <button onClick={onLangToggle} className="text-xs font-mono px-3 py-1.5 rounded-md border border-white/10 text-white/50">
              {lang === "ko" ? "EN" : "한"}
            </button>
            <Button
              onClick={() => scrollTo("pricing")}
